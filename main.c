...
static inline void I2C_start(void){
  SDA_out(); SDA_high(); SCL_high(); i2c_delay(); SDA_low(); i2c_delay(); SCL_low(); i2c_delay();
}
static inline void I2C_stop(void){
  SDA_out(); SDA_low(); i2c_delay(); SCL_high(); while(!SCL_read()){} i2c_delay(); SDA_high(); i2c_delay();
}
static inline int I2C_wait_ack(void){
  SDA_in(); SCL_high(); for(volatile int t=0;t<30000 && !SCL_read();++t){} i2c_delay();
  int nack = SDA_read(); SCL_low(); i2c_delay(); SDA_out(); SDA_high(); return nack;
}
static inline void I2C_write8_raw(uint8_t v){
  SDA_out();
  for(int i=0;i<8;i++){ if(v&0x80) SDA_high(); else SDA_low(); i2c_delay(); SCL_high(); while(!SCL_read()){} i2c_delay(); SCL_low(); i2c_delay(); v<<=1; }
  SDA_high();
}
static inline uint8_t I2C_read8_raw(int ack){
  uint8_t d=0; SDA_in();
  for(int i=0;i<8;i++){ d<<=1; SCL_high(); while(!SCL_read()){} i2c_delay(); if(SDA_read()) d|=1; SCL_low(); i2c_delay(); }
  SDA_out(); if(ack) SDA_low(); else SDA_high(); i2c_delay(); SCL_high(); while(!SCL_read()){} i2c_delay(); SCL_low(); i2c_delay(); SDA_high(); return d;
}
...
int bme_write8(uint8_t reg, uint8_t val){
  I2C_start(); I2C_write8_raw(BME_I2C_ADDR_WR); if(I2C_wait_ack()){ I2C_stop(); return 1; }
  I2C_write8_raw(reg); if(I2C_wait_ack()){ I2C_stop(); return 2; }
  I2C_write8_raw(val); if(I2C_wait_ack()){ I2C_stop(); return 3; }
  I2C_stop(); return 0;
}
int bme_read8(uint8_t reg, uint8_t *out){
  ...
  I2C_start(); I2C_write8_raw(BME_I2C_ADDR_WR); if(I2C_wait_ack()){ I2C_stop(); return 1; }
  I2C_write8_raw(reg); if(I2C_wait_ack()){ I2C_stop(); return 2; }
  I2C_start(); I2C_write8_raw(BME_I2C_ADDR_RD); if(I2C_wait_ack()){ I2C_stop(); return 3; }
  *out = I2C_read8_raw(0); I2C_stop(); return 0;
}

static void delay_ms_soft(unsigned ms){ for(unsigned i=0;i<ms;i++){ for(volatile unsigned k=0;k<8000;k++) __NOP(); } }
